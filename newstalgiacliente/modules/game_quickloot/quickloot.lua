    ...
    Keybind.bind("Loot", "Quick Loot Nearby Corpses", {
      {
        type = KEY_DOWN,
        callback = function()
            if isPremiumPlayer() then
                g_game.sendQuickLoot(2)
            else
                displayInfoBox(tr('Premium Account'), tr('You need a premium account to use quickloot.'))
            end
        end,
      }
    })
        ...
        if mouseButton == MouseLeftButton then
            ...
            if clickedWidget then
                ...
                elseif clickedWidget:getClassName() == "UIItem" and not clickedWidget:isVirtual() then
                    if clickedWidget:getItem() and clickedWidget:getItem():isContainer() and isPremiumPlayer() then
