...
local function onCharacterList(protocol, characters, account, otui)
    local httpLogin = not G.host:find("^https://")
...
local function updateLabelText()
    if enterGame:getChildById('clientComboBox') and tonumber(enterGame:getChildById('clientComboBox'):getText()) > 1080 then
        enterGame:setText("Welcome to Newstalgia")
...
function EnterGame.postCacheInfo()
    ...
    local onRecvInfo = function(message, err)
        ...
        modules.client_topmenu.setPlayersOnlineDevices(
            response.players_desktop,
            response.players_tablet,
            response.players_mobile
        )
    ...
    HTTP.post(Services.status, json.encode({
        type = requestType,
        device = g_platform.deviceCategory()
    }), onRecvInfo, false)
end
...
function EnterGame.setUniqueServer(host, port, protocol, windowWidth, windowHeight)
    ...
    for _, widgetId in pairs({
        'stayLoggedBox', 'serverLabel', 'serverListButton', 'serverHostTextEdit',
        'clientLabel', 'clientComboBox', 'portLabel', 'serverPortTextEdit',
        'autoLoginBox', 'httpLoginBox'
    }) do
        local widget = enterGame:getChildById(widgetId)
