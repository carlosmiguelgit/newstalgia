...
function Cyclopedia.toTitleCase(name)
    if not name or name == '' then
        return name
    end
    return name:gsub("(%S+)(%s*)", function(word, spaces)
        return word:sub(1, 1):upper() .. word:sub(2) .. spaces
    end)
end
...
function controllerCyclopedia:onInit()
    ...
    self:registerEvents(g_game, {
        ...
        onParseCyclopediaHouseList = function(data)
            if Cyclopedia.loadHouseList then
                Cyclopedia.loadHouseList(data)
            end
        end,
        onParseCyclopediaHouseAuctionMessage = function(houseId, typeValue, index)
            if Cyclopedia.houseMessage then
                Cyclopedia.houseMessage(houseId, typeValue, index)
            end
        end,
        onParseCyclopediaHousesInfo = function(houseClientId, accountHouseCount, houseIds)
            if Cyclopedia.loadHousesInfo then
                Cyclopedia.loadHousesInfo(houseClientId, accountHouseCount, houseIds)
            end
        end
    })
end
...
function Cyclopedia.Toggle(force, hidePanel, windowType)
    if not controllerCyclopedia.ui then
        return
    end

    local tab = windowType
    if type(windowType) == "number" then
        local tabs = { "items", "bestiary", "charms", "map", "houses", "character", "bosstiary", "bossSlot",
            "magicalArchives" }
        tab = tabs[windowType]
    end

    if tab and windowTypes[tab] then
        SelectWindow(tab, false)
        return
    end
