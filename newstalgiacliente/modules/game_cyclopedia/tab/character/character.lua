...
function Cyclopedia.loadCharacterItems(data)
    ...
    local function insert(data, type)
        ...
        local name = Cyclopedia.toTitleCase(thing:getMarketData().name)
...
function Cyclopedia.loadCharacterCombatStats(data, mitigation, additionalSkillsArray, forgeSkillsArray,
    ...
    if concoctionsArray or next(concoctionsArray) ~= nil then
        for i = 1, #concoctionsArray do
            ...
            local itemName = Cyclopedia.toTitleCase(widget.item:getItem():getMarketData().name)
        ...
        if button.subCategories ~= nil then
            ...
            for subId, subButton in ipairs(subCats) do
                ...
                function subWidget.Button:onClick(test)
                    ...
                    elseif subWidget.open == "CharacterTitles" then
...
function Cyclopedia.loadCharacterTitles(currentTitle, titles)
    Cyclopedia.titleCurrent = currentTitle
    Cyclopedia.titleCache = {}

    for _, data in ipairs(titles) do
        table.insert(Cyclopedia.titleCache, { id = data[1], name = data[2], description = data[3], permanent = data[4], unlocked = data[5] })
    end
    table.sort(Cyclopedia.titleCache, function(a, b) return a.name < b.name end)

    Cyclopedia.renderCharacterTitles()
end

function Cyclopedia.renderCharacterTitles()
    if not UI or not UI.CharacterTitles or not Cyclopedia.titleCache then
        return
    end

    local filters = UI.CharacterTitles.titleFilters
    ...
    for i = 1, filters:getChildCount() do
        ...
        if child:isChecked() then
            activeFilter = child:getId()
        end
    end
    local searchText = filters.searchEdit:getText():lower()
    local currentTitle = Cyclopedia.titleCurrent
    local colorAlt = { "#404040", "#484848" }
    local colorIndex = 1

    for _, title in ipairs(Cyclopedia.titleCache) do
        local show = true
        if activeFilter == "filterPermanent" then
            show = title.permanent
        elseif activeFilter == "filterUnlocked" then
            show = title.unlocked
        elseif activeFilter == "filterTemporary" then
            show = not title.permanent
        elseif activeFilter == "filterLocked" then
            show = not title.unlocked
        end

        if show and searchText ~= "" and not string.find(title.name:lower(), searchText, 1, true) then
            show = false
        end

        if show then
            local item = g_ui.createWidget("CharacterTitleItem", list)
            item.name:setText(title.name)
            item:setBackgroundColor(colorAlt[colorIndex])
            colorIndex = colorIndex == 1 and 2 or 1
            item.permanentIcon:setImageSource(title.permanent and "/images/ui/icon-yes" or "/images/ui/createaccount/icon-no")
            item.unlockedIcon:setImageSource(title.unlocked and "/images/ui/icon-yes" or "/images/ui/createaccount/icon-no")
            if title.description and title.description ~= "" then
                item:setTooltip(title.description)
            end

            local isCurrent = title.id == currentTitle
            item.name:setColor(isCurrent and "#E0C060" or "#C0C0C0")

            if title.unlocked then
                if isCurrent then
                    item.selectButton:setText(tr('Deselect'))
                    item.selectButton:setTooltip(tr('Remove character title'))
                    item.selectButton.onClick = function() g_game.updateCharacterTitle(0) end
                else
                    item.selectButton:setText(tr('Select'))
                    item.selectButton:setTooltip(tr('Set as character title'))
                    item.selectButton.onClick = function() g_game.updateCharacterTitle(title.id) end
                end
                item.selectButton:setVisible(true)
            else
                item.selectButton:setVisible(false)
            end
        end
    end

    local currentTitleLabel = UI.CharacterTitles.currentTitle.titleLabel
    local found = false
    for _, title in ipairs(Cyclopedia.titleCache) do
        if title.id == currentTitle then
            currentTitleLabel:setText(title.name)
            currentTitleLabel:setColor("#E0C060")
            found = true
            break
        end
    end
    if not found then
        currentTitleLabel:setText(tr('None'))
        currentTitleLabel:setColor("#C0C0C0")
    end

    local clearTitleButton = UI.CharacterTitles.currentTitle.clearTitleButton
    if clearTitleButton then
        clearTitleButton:setVisible(found)
    end
end

function Cyclopedia.titleFilter(widget)
    if not UI or not UI.CharacterTitles then return end
...
function Cyclopedia.titleSearch(text)
    Cyclopedia.renderCharacterTitles()
end

function Cyclopedia.titleClearSearch(button)
    if not UI or not UI.CharacterTitles then return end
    UI.CharacterTitles.titleFilters.searchEdit:setText("")
    Cyclopedia.renderCharacterTitles()
end

function Cyclopedia.clearCurrentTitle()
            ...
            for _, v in ipairs(data.concoctions) do
                ...
                if thing then
                    ...
                    if marketData and marketData.name and marketData.name ~= "" then
                        itemName = Cyclopedia.toTitleCase(marketData.name)
                    end
                end
