...
function showHouse()
    ...
    Cyclopedia.buildCityList()

    Cyclopedia.House.Loaded = false
    Cyclopedia.House.lastChangeState = nil
...
function Cyclopedia.buildCityList()
    if Cyclopedia.CityList.built then
        return
    end

    if not HOUSE_TOWNS or not TOWNS then
        return
    end

    for i = 1, #HOUSE_TOWNS do
        Cyclopedia.CityList[i] = { Title = TOWNS[HOUSE_TOWNS[i]] }
    end

    Cyclopedia.CityList.built = true
end
...
local function resetButtons()
    ...
    if UI.LateralBase:getChildById("cancelMoveOut") then
        UI.LateralBase:getChildById("cancelMoveOut"):destroy()
    end
...
function Cyclopedia.houseChangeState(widget)
    if Cyclopedia.House.Data then
        ...
        local option
        if widget and not widget:isDestroyed() and widget.getCurrentOption then
            option = widget:getCurrentOption()
        elseif UI and UI.TopBase and UI.TopBase.StatesOption then
            option = UI.TopBase.StatesOption:getCurrentOption()
        end
        local type = option and option.data or 1
        for _, data in ipairs(Cyclopedia.House.Data) do
            local stateVisible
            if type == 3 then
                stateVisible = data.rented or data.inTransfer
            elseif type == 2 then
                stateVisible = not data.rented
            else
                stateVisible = true
            end

            if onlyGuildHall then
                data.visible = stateVisible and data.gh
            else
                data.visible = stateVisible and not data.gh
            end
        end
...
function Cyclopedia.houseMessage(houseId, type, message)
    ...
    if type == 1 then
        ...
        elseif message == 5 then
            confirmWindow = displayGeneralBox(tr("Summary"), tr(
                "Bid failed.\nYour character needs premium time to bid on houses."),
                {
                    {
                        text = tr("Ok"),
                        callback = yesCallback
    ...
    elseif type == 4 then
        if message == 0 then
            confirmWindow = displayGeneralBox(tr("Summary"), tr(
                "You have successfully cancelled your move out. You will keep the house."), {
                {
                    text = tr("Ok"),
                    callback = yesCallback
                },
...
function Cyclopedia.cancelMoveOutHouse()
    local house = Cyclopedia.House.lastSelectedHouse.data
    local confirmWindow
    ...
    local function yesCallback()
        ...
        g_game.requestCancelMoveOutHouse(house.id)
        Cyclopedia.House.ignore = true
        UI.TopBase.StatesOption:setOption("All States", true)
        UI.TopBase.SortOption:setOption("Sort by name", true)
    end
    ...
    if not confirmWindow then
        confirmWindow = displayGeneralBox(tr("Confirm House Action"),
            tr("Do you really want to cancel moving out of the house '%s'?\nYou will keep the house.", house.name), {
        ...
        elseif type == 4 then
            table.sort(Cyclopedia.House.Data, function(a, b)
                ...
                elseif not a.hasBid and b.hasBid then
                    return false
                elseif a.hasBid and b.hasBid then
                    return (a.hightestBid or 0) > (b.hightestBid or 0)
                else
                    return a.name < b.name
                end
            end)
        ...
        for _, data in ipairs(Cyclopedia.House.Data) do
            if data.visible then
                ...
                elseif data.state == 3 then
                    widget.status:setColoredText("{Status:  , #909090}transfer to " ..
                                                     (data.transferName and data.transferName or "?") ..
                                                     " on " .. os.date("%b %d", data.transferTime))
                elseif data.state == 4 then
                    widget.status:setColoredText("{Status:  , #909090}moving out on " ..
                                                     (data.bidEnd and os.date("%b %d", data.bidEnd) or "?"))
                end
...
function Cyclopedia.loadHouseList(data)
        ...
        for i = 1, #data do
            ...
            if house then
                ...
                local data_t = {
                    ...
                    state = value.state,
                    owner = value.owner and value.owner ~= "" and value.owner or "?",
                    isYourBid = value.isBidder and true or false,
                    hasBid = value.bidEnd and value.bidEnd > 0 and true or false,
                    bidEnd = value.bidEnd and value.bidEnd > 0 and value.bidEnd or nil,
                    hightestBid = value.hightestBid or nil,
                    bidName = value.bidName or nil,
                    bidHolderLimit = value.bidHolderLimit or nil,
                    canBid = value.canBid or 0,
                    rented = value.state == 2 and true or false,
                    paidUntil = value.paidUntil or nil,
                    isYourOwner = value.isYourOwner and true or false,
                    inTransfer = value.state == 3 and true or false,
                    inMoveOut = value.state == 4 and true or false,
                    transferName = value.transferPlayer or nil,
                    transferTime = value.time or 0,
                    transferValue = value.transferValue or 0,
                    isTransferOwner = value.isTransferOwner and true or false,
                    canAcceptTransfer = value.canAcceptTransfer or 0
                }
...
function Cyclopedia.loadHousesInfo(houseClientId, accountHouseCount, houseIds)
    Cyclopedia.House.myHouseClientId = houseClientId
    Cyclopedia.House.accountHouseCount = accountHouseCount
    Cyclopedia.House.houseIds = houseIds
end
...
function Cyclopedia.selectHouse(widget)
    ...
    elseif widget.data.inMoveOut and widget.data.isYourOwner then
        local button = g_ui.createWidget("Button", UI.LateralBase)
        button:setId("cancelMoveOut")
        button:setText("Cancel Move Out")
        button:setColor("#C0C0C0")
        button:setWidth(96)
        button:setHeight(20)
        button:addAnchor(AnchorBottom, "parent", AnchorBottom)
        ...
        button:setMarginRight(7)
        button:setMarginBottom(7)
        button.onClick = Cyclopedia.cancelMoveOutHouse
    ...
    UI.LateralBase.MapViewbase.houseImage:setImageSource("/game_cyclopedia/images/houses/generic")
