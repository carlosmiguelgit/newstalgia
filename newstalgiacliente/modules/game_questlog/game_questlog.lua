...
local function showQuestTracker(autoOpen)
...
function questLogController:onGameStart()
    if g_game.getClientVersion() >= 1280 then
        ...
        if trackerMiniWindow then
            ...
            -- Never auto-open the tracker on client start, even if it was open
            -- in a previous session. The button still opens it on demand.
            if trackerMiniWindow:isExplicitlyVisible() then
