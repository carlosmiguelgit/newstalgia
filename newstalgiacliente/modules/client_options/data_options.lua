local function setClassicControl(options, panels)
    options.classicControl.value = true
    options.smartLeftClick.value = false
    options.mouseControlMode.value = 1
    g_settings.set('classicControl', true)
    g_settings.set('smartLeftClick', false)
    g_settings.set('mouseControlMode', 1)

    scheduleEvent(function()
        local mouseControlModeCombobox = panels.generalPanel and panels.generalPanel:recursiveGetChildById('mouseControlMode')
        if mouseControlModeCombobox then
            mouseControlModeCombobox:setCurrentOptionByData(1, true)
        end

        local lootControlModeCombobox = panels.generalPanel and panels.generalPanel:recursiveGetChildById('lootControlMode')
...
return {
    ...
    classicControl                    = {
        value = true,
        action = function(value, options, controller, panels, extraWidgets)
            setClassicControl(options, panels)
        end
    },
    smartLeftClick                    = {
        value = false,
