...
local function setup()
    ...
    -- Special handling for mouseControlMode to ensure it always stays as Classic Controls
    setOption('mouseControlMode', 1, true)

    -- Fixed values (fields removed from the UI, not changeable by the client)
    ...
    setOption('floorViewMode', 0, true)
...
function updatePremiumLootOption()
    if not panels.generalPanel then
        return
    end
    local checkbox = panels.generalPanel:recursiveGetChildById('lootOnRightClick')
    ...
    if player and player:isPremium() then
        ...
        setOption('lootOnRightClick', false, true)
    end
end
