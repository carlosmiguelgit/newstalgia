...
local function load(version)
    ...
    if version >= 1281 and not g_game.getFeature(GameLoadSprInsteadProtobuf) then
        ...
        if g_game.getFeature(GameLoadSprInsteadProtobuf) and version >= 1281 then
            ...
            local thingsPath = resolvepath(string.format('/data/things/%d/', version))
            if not g_things.loadStaticData(thingsPath) then
                g_logger.warning(string.format(
                    "[game_things.load()] Couldn't load /data/things/%d/staticdata, bestiary images won't be available.", version))
            end
            if g_game.getFeature(GameProficiency) and not g_things.resolveProficienciesFile(thingsPath) then
                g_logger.warning(string.format(
                    "[game_things.load()] Couldn't load /data/things/%d/proficiencies-<hash>.json", version))
            end
        end
