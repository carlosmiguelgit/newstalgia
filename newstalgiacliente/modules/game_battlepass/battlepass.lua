...
local BATTLE_PASS_MINIMUM_LEVEL = 8
local BATTLE_PASS_LEVEL_ERROR = "You need level 8 to use the Battle Pass."

local function rewardKey(step, rewardType)
    return rewardType .. ':' .. step
end

-- Network protocol for this server: client -> server 0x36, server -> client 0x39.
...
function BattlePass.redirectToStore()
    ...
    g_game.requestStoreOffers("Battle Pass", "", 0, 1)
end
...
openBattlePass = function()
    ...
    elseif not g_game.isOnline() then
        ...
        local player = g_game.getLocalPlayer()
        local level = player and player.getLevel and player:getLevel()
        if level and tonumber(level) < BATTLE_PASS_MINIMUM_LEVEL then
            displayErrorBox(tr("Battle Pass"), BATTLE_PASS_LEVEL_ERROR)
            return
        end
...
onResourceBalance = function(value, oldBalance, resourceType)
    if resourceType and resourceType ~= ResourceTypes.BANK_BALANCE and resourceType ~= ResourceTypes.GOLD_EQUIPPED then
        return
    end
