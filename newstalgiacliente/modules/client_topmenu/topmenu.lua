...
function init()
    ...
    topLeftDevicesLabel = topMenu:recursiveGetChildById('topLeftDevicesLabel')
...
function setPlayersOnlineDevices(desktop, tablet, mobile)
    if not topLeftDevicesLabel then
        return
    end
    local d = type(desktop) == 'number' and desktop or 0
    local t = type(tablet) == 'number' and tablet or 0
    local m = type(mobile) == 'number' and mobile or 0
    topLeftDevicesLabel:setText(('Desktop %d | Tablet %d | Mobile %d'):format(d, t, m))
    topLeftDevicesLabel:show()
end
