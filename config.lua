...
-- NOTE: logPlayerIP will create a log file of characters and their IP, this can be useful to enforce your server rules.
logLevel = "info"
logPlayerIP = false
...
-- NOTE forgeInfluencedIntervalTime and forgeInfluencedIntervalType: time and type of interval to limit the influenced monsters, for example, if you set forgeInfluencedIntervalTime to 1 and forgeInfluencedIntervalType to "hour", only 1 influenced monster will be created per hour, if the limit is reached, no more influenced monsters will be created until the next hour.
forgeMaxItemTier = 10
...
forgeInfluencedIntervalType = "minute"
forgeInfluencedIntervalTime = "30"
...
-- NOTE: advancedPartyProtection will nullfy the damage along your party members and their summons
...
timeBetweenNecklace = 500
timeBetweenRing = 500
timeAfterParalyze = 500
...
-- Compressed Map
-- NOTE: toggleCompressedMap must be true to load compressed .otbm maps (RME "Optimized map save")
toggleCompressedMap = true
...
-- Store
-- NOTE: storeCombinedCoins = true uses transferable coins first, then regular coins for store purchases.
-- NOTE: When storeCombinedCoins = false, storeCoinType selects which coin type is used (0 = regular, 1 = transferable).
coinPacketSize = 25
coinImagesURL = "http://127.0.0.1/images/store/"
storeCombinedCoins = true
storeCoinType = 0
...
-- Exiva Restrictions
-- NOTE: exivaRestrictionsOnlyOptionalWorlds = true to enable exiva restrictions only on optional worlds
-- NOTE: maxExivaWhitelist = maximum number of players that can be whitelisted for exiva
exivaRestrictionsOnlyOptionalWorlds = true
...
-- NOTE: rateWeaponProficiency multiplies Weapon Proficiency experience gained from kills
...
-- Mounts
-- NOTE: set mountSounds to true will display onomatopoeia creature message from your selected mount
