-- Battle Pass module for the Crystal Server network modules system.
-- Protocol: client -> server byte 0x36, server -> client byte 0x39.
-- NOTE: 0x36/0x37 are used natively by the shader system (sendShader/sendMapShader);
-- 0x36 stays free on C->S, so the response opcode was moved to 0x39 (custom range 51-99).
-- See data/XML/storages.xml for the storage range (90500 - 91000).

dofile(CORE_DIRECTORY .. "/modules/scripts/battlepass/config.lua")
...
local BATTLEPASS_SEND_OPCODE = 0x39
...
-- Storage keys (raw numeric, same values declared in data/XML/storages.xml)
local STORAGE_SEASON_EPOCH = 90500
local STORAGE_SEASON_STARTED_AT = 90501
local STORAGE_DOUBLE_SKILL_UNTIL = 90731
...
if type(config) ~= "table" then
	error("Battle Pass configuration was not loaded. Check data/modules/scripts/battlepass/config.lua")
end
...
local function getSeason()
	...
	local epoch = tonumber(Game.getStorageValue(STORAGE_SEASON_EPOCH)) or -1
	local beginTime = tonumber(Game.getStorageValue(STORAGE_SEASON_STARTED_AT)) or -1
	...
	if epoch < 1 or beginTime < 1 then
		...
		Game.setStorageValue(STORAGE_SEASON_EPOCH, epoch)
		Game.setStorageValue(STORAGE_SEASON_STARTED_AT, beginTime)
	end
...
local function getItemTypeInfo(itemId)
	...
	-- This data pack uses the item id directly as the client appearance id.
...
local function deliverReward(player, reward, objectId)
	...
	elseif definition.type == "doubleSkill" then
		local now = os.time()
		local currentUntil = tonumber(player:getStorageValue(STORAGE_DOUBLE_SKILL_UNTIL)) or 0
		player:setStorageValue(STORAGE_DOUBLE_SKILL_UNTIL, math.max(now, currentUntil) + reward.durationTime * 60 * 60)
		return true
...
function BattlePassSystem.startNewSeason()
	local currentEpoch = tonumber(Game.getStorageValue(STORAGE_SEASON_EPOCH)) or 0
	Game.setStorageValue(STORAGE_SEASON_EPOCH, math.max(1, currentEpoch + 1))
	Game.setStorageValue(STORAGE_SEASON_STARTED_AT, os.time())
...
function onRecvbyte(player, msg, byte)
	if byte ~= BATTLEPASS_REQUEST_OPCODE then
		return true
	end
	...
	elseif request == REQUEST_REDEEM then
		local index = msg:getU16()
		local rewardId = msg:getU32()
		local objectId = msg:getU32()
		if index == nil or rewardId == nil or objectId == nil then
